        ...
        for _ in range(self.args.rollout):  # 这个rollout控制的是选择次数，如果从根节点开始，第一次选第一层，第二次可能选的是第二层，第三次选第三层
            ...
            if not node.is_terminal:
                if self.args.mctsvalue == 'gpteval':
                    ...
                    """
                    if node.decision_memory != '' and self.args.mctsvalue == 'verbalMemoHistory':
                        memory_ids = self.tokenizer.encode(node.decision_memory)
                        with_memory_state = state + memory_ids
                        code_id = self.generator.get_rationale_predicted_sequence(with_memory_state)
                    else:
                        code_id = self.generator.get_rationale_predicted_sequence(state)
                    ...
                    if failed_tests != '':
                        ...
                        verbalMemory = (f"{self.tokenizer.decode(state)}\n\n {code}\n\n"
                                        f"Above is the combination of problem + thoughts + code. The code is generated following the thoughts to solve the problem. "
                        ...
                        for k, cur_failed_test in enumerate(failed_test_list):  # 这里用k就可以了，不再需要tmp count，因为有问题的没有加入到list里面
                            ...
                            if trace_block_texts != '':
                                ...
                                break
                        if len(self.tokenizer.encode(verbalMemory, allowed_special={'<|endoftext|>'})) > 12000:
                            tmp_shorter = self.tokenizer.encode(verbalMemory, allowed_special={'<|endoftext|>'})[:8000]
                            verbalMemory = self.tokenizer.decode(tmp_shorter)
                        # if node.decision_memory == '':
                        #     node.decision_memory = '\nHere are the history generated failed codes. Try to avoid the mistakes in the history.\n'
                        # if len(self.tokenizer.encode(node.decision_memory)) < 2048:
                        #     node.decision_memory += f"\n**        Failed Trail        **\n"
                        #     node.decision_memory += f"{code}\n{failed_tests}\n"
...
class MineDecisionNode:
    ...
    def __init__(self, parent, state, is_terminal=False, generator=None, id=None, decision_memory=''):
        self.id = id
        self.parent = parent
        self.state = state
    ...
    def __expand__(self, verbal_feedback=''):
        ...
        self.children = [MineChanceNode(self, (act, score), chance_memory=self.decision_memory) for act, score in zip(self.possible_actions, self.action_scores)]
