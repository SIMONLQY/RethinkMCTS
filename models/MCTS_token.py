    ...
    def mcts_procedure(self, initial_state, done):
        ...
        for _ in range(self.args.rollout):  # 这个rollout控制的是选择次数，如果从根节点开始，第一次选第一层，第二次可能选的是第二层，第三次选第三层
            ...
            if type(node) == ChanceNode:
                ...
                node.children.append(DecisionNode(node, state_p, terminal, generator=self.generator, id=decision_node_num))  # chance node 只有一个子节点，就是加上了那个动作的节点,但每一个decision node在创建的时候都会带有3个可能的动作
                decision_node_num += 1
